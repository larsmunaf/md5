...
#define FF(a,b,c,d,M_j,s,t_i) (a = b + lrot((a + F(b,c,d) + M_j + t_i),s))
#define GG(a,b,c,d,M_j,s,t_i) (a = b + lrot((a + G(b,c,d) + M_j + t_i),s))
#define HH(a,b,c,d,M_j,s,t_i) (a = b + lrot((a + H(b,c,d) + M_j + t_i),s))
#define II(a,b,c,d,M_j,s,t_i) (a = b + lrot((a + I(b,c,d) + M_j + t_i),s))
...
unsigned char* padding (unsigned char* plainText)
{
    unsigned char* paddedPlainText = malloc (BLKSIZE); // use a buffer not to change the original message
    strcpy ((char*) paddedPlainText, (char*) plainText);
    uint32_t bytesWritten = strlen ((char*) paddedPlainText);
    uint64_t oldLength = bytesWritten * 8; // length before padding is needed to append the length representation in bits !!!

//    /* add the 1-bit */
//    setByteOfString (paddedPlainText, bytesWritten++, 1);
    *(paddedPlainText + (bytesWritten++)) = 0x80;

    /* add padding zeros */
    while (bytesWritten < 64) // iterate through the whole block instead of stopping 64 bits before the end
    {
        *(paddedPlainText + (bytesWritten++)) = 0x00;
    }
...
/* step 5: output */
void output (uint32_t a, uint32_t b, uint32_t c, uint32_t d)
{
    a = swapByte32 (a), b = swapByte32 (b), c = swapByte32 (c), d = swapByte32 (d);
...
int main ()
{
    ...
    unsigned char* message1 = (unsigned char*) "PLEGAR17A1218348";

    printf ("message: %s\n", message1);
    /* step 1 + 2: append padding bits */
    result = padding (message1);
    /* step 3: initialize MD buffer */
    initMDBuffer ();
    /* step 4: main MD5 function */
    md5 (result);
